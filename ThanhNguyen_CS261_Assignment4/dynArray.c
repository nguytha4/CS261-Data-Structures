...
TYPE getMinHeap(DynArr *heap)
{
	...
	/* Assign temp to the first datapoint in the array and return it */
  	TYPE temp = heap->data[0];
  	return temp;
}
...
void addHeap(DynArr *heap, TYPE node)
{
	...
	/* Create variables to hold the position of the variable being added and compute its parent's position */
	...
	/* Percolat the variable up as needed by swapping and recomputing the child and parent */
	while (pos != 0)
	{
		parent = (pos - 1) / 2;
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	if (rghtIdx < max)
	{
		/* Compute which child is smaller */
		int smallest = _smallerIndexHeap(heap, leftIdx, rghtIdx);

		/* If value as pos is greater than smallest child */
		if (compare(getDynArr(heap, pos), getDynArr(heap, smallest)) == 1)
		{
			/* Swap */
			swapDynArr(heap, pos, smallest);

			/* Recursively call _adjustHeap */
	...
	{
		/* If value as pos is greater than child */
		if (compare(getDynArr(heap, pos), getDynArr(heap, leftIdx)) == 1)
		{
			/* Swap */
...
void _buildHeap(DynArr *heap)
{
	...
	/* For loop to build the heap; addressing each subtree in reverse order */
...
void sortHeap(DynArr *heap)
{
	...
	/* Build a heap from a randomly sorted array */
	_buildHeap(heap);

	/* For loop to sort the array in reverse order. */
	for (int i = sizeDynArr(heap) - 1; i > 0; i--)
	{
		swapDynArr(heap, 0, i);		/* Swap the first and last elements */
		_adjustHeap(heap, i, 0);	/* Percolate the element down */
	}
}
