...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	// If the source is the same as the destination, return 1
	if (source == destination)
		return 1;

	// Create a pointer to a deque stack
	Deque * dfsDeque = dequeNew();

	// Create vertex placeholder variable to check its value and neighbors
	Vertex * curVertex;

	// Add source to the stack to start
	dequePushFront(dfsDeque, source);

	// While stack is not empty
	while (!dequeIsEmpty(dfsDeque))
	{
		// Set the placeholder vertex to the top of the stack
		curVertex = dequeFront(dfsDeque);

		// Pop the top value off the stack
		dequePopFront(dfsDeque);

		// If the popped vertex is not already marked as reachable
		if(!curVertex->isVisited)
		{
			// Make sure it is marked as reacheable
			curVertex->isVisited = 1;

			// Look at the neighbors using a for loop
			for (int i = 0; i < curVertex->numNeighbors; i++)
			{
				// If the destination is reached in the form of a neighbor.
				if (curVertex->neighbors[i] == destination)
				{
					dequeDelete(dfsDeque);		// Delete the stack
					return 1;					// Return connected
				}

				// If the neighbors of the current vertex have not been marked as reachable, add them to the stack
				if (!curVertex->neighbors[i]->isVisited)
				{
					dequePushFront(dfsDeque, curVertex->neighbors[i]);
				}
			}
		}
	}
	...
	// Delete the stack
	dequeDelete(dfsDeque);

	// If after all of the reachable vertices have been explored and destination still has not been reached, return not connected
    return 0;
}
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	// Create a pointer to a deque queue
	Deque * bfsDeque = dequeNew();

	// Create vertex placeholder variable to check its value and neighbors
	Vertex * curVertex;

	// Add source to the queue to start (from the back)
	dequePushBack(bfsDeque, source);

	// While queue is not empty
	while (!dequeIsEmpty(bfsDeque))
	{
		// Set the placeholder vertex to the front of the queue
		curVertex = dequeFront(bfsDeque);

		// Remove the value at the front of the queue
		dequePopFront(bfsDeque);
		...
		if (!curVertex->isVisited)
		{
			...
			for (int i = 0; i < curVertex->numNeighbors; i++)
			{
				// If the destination is reached in the form of a neighbor. Return connected
				if (curVertex->neighbors[i] == destination)
				{
					dequeDelete(bfsDeque);		// Delete the queue
					return 1;					// Return connected
				}

				// If the neighbors of the current vertex have not been marked as reachable, add them to the back of the queue
				if (!curVertex->neighbors[i]->isVisited)
				{
					dequePushBack(bfsDeque, curVertex->neighbors[i]);
				}
			}
		}
	}
